...
**`.hic` File Compatibility**:

Quagga uses the modern `hic-straw` library to read .hic files, which supports all file format versions including those generated by Juicer 2.0+. Previously, `hic_straw.py` only supported version 1.6, and failed with a `TypeError: 'int' object is not subscriptable`.

Your .hic files must contain pre-computed normalization vectors (KR, VC, etc.) for balanced normalization to work. If your files lack these vectors, add them:

```bash
java -jar juicer_tools.jar addNorm -k KR file.hic
```
